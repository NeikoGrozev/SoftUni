function solve(input){
    ...
    for (const line of input) {
        ...
        let [system, component, subs] = line.split(' | ');

        if(!register.hasOwnProperty(system)){
            register[system] = {};
        }

        if(register.hasOwnProperty(system) && !register[system].hasOwnProperty(component)){
            register[system][component] = [];
        }

        register[system][component].push(subs);
    }

    Object.entries(register).sort((a, b) =>{
